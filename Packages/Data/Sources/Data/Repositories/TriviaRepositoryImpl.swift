...
public struct TriviaRepositoryImpl: TriviaRepository {
    ...
    public func fetchQuestions(settings: QuizSettings, policy: CachePolicy) async throws -> [QuizQuestion] {
        let now = Date()
    ...
    public func saveResult(_ result: QuizResult) async {
        let item = HistoryItem(id: UUID().uuidString, result: result)
        await historyStore.append(item)
    }

    public func fetchHistory() async -> [HistoryItem] {
