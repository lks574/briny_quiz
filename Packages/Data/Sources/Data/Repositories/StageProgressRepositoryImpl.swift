...
public final class StageProgressRepositoryImpl: StageProgressRepository {
    private let store: StageProgressStore
    private let packRepository: PackRepository

    public init(packRepository: PackRepository) {
        self.store = StageProgressStore()
        self.packRepository = packRepository
    }

    init(store: StageProgressStore, packRepository: PackRepository) {
        self.store = store
        self.packRepository = packRepository
    }

    public func fetchProgress() async -> [StageProgress] {
        await store.load()
    }

    public func updateProgress(stageId: String, score: Int) async {
